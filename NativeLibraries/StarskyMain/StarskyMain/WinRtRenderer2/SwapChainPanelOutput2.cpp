...
void SwapChainPanelOutput2::CreateWindowSizeDependentResources() {
	...
	D2D1_BITMAP_PROPERTIES1 bitmapProperties =
		D2D1::BitmapProperties1(
			...
			D2D1::PixelFormat(SwapChainPanelOutput2::BufferFmt, D2D1_ALPHA_MODE_PREMULTIPLIED),
			this->logicalDpi,
			this->logicalDpi
		);
	...
	if (this->swapChain) {
		this->swapChain->ResizeBuffers(
			SwapChainPanelOutput2::BufferCount,
			lround(this->physicalSize.x),
			lround(this->physicalSize.y),
			SwapChainPanelOutput2::BufferFmt,
			0);
	}
	else {
...
void SwapChainPanelOutput2::CreateSwapChain() {
	...
	swapChainDesc.Format = SwapChainPanelOutput2::BufferFmt;				// This is the most common swap chain format.
	swapChainDesc.Stereo = false;
	swapChainDesc.SampleDesc.Count = 1;								// Don't use multi-sampling.
	swapChainDesc.SampleDesc.Quality = 0;
	swapChainDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
	swapChainDesc.BufferCount = SwapChainPanelOutput2::BufferCount;									// Use double-buffering to minimize latency.
...
void SwapChainPanelOutput2::UpdatePresentationParameters() {
